...
def find_characters(sentences):
    characters_list = []
    words = ' '.join([i[1:].strip() for i in sentences]).split()
    parts_disable = {"PRON", "ADV", "CONJ", "PRED", "PRCL", "NPRO", "INTJ", "DET", "X"}
    parts_available = {"INF", "VERB", "INFN"}
    for i in range((len(words) - 1)):
        parts_of_speech_next = set(wordtags[words[i + 1].lower()].keys())
        parts_of_speech_this = set(wordtags[words[i].lower()].keys())
